...
def get_cindex(y_true, y_pred):
    """
    Efficient CI computation for DTA-style regression.
    Excludes pairs with tied y_true.
    """
    ...
    pred_rank_map = {v: i + 1 for i, v in enumerate(unique_pred)}  # 1-based for Fenwick
    pred_ranks = np.array([pred_rank_map[v] for v in y_pred], dtype=np.int64)

    # sort by y_true ascending
    ...
    while start < n:
        ...
        # add current group into BIT after evaluation
        for k in range(start, end):
...
def main():
    ...
    # train
    parser.add_argument("--output_dir", type=str, default="outputs/davis_v1")
