...
def parse_args():
    ...
    p.add_argument("--train_script", type=str, default="train_p13d_softclip_pcl.py")
    ...
    p.add_argument("--affinity_temperatures", type=float, nargs="+", default=[1.0])
    p.add_argument("--pcl_temperature_inits", type=float, nargs="+", default=[0.07])
    p.add_argument("--sweep_name", type=str, default="")
    p.add_argument("--output_root", type=str, default="outputs/softclip_pcl_sweeps")
...
def make_sweep_dir(args):
    name = args.sweep_name.strip() or ("softclip_pcl_sweep_" + datetime.now().strftime("%Y%m%d_%H%M%S"))
...
def build_experiments(args):
    exps = []
    for bs, lr, lc, lp, temp, aff_temp, pcl_temp in itertools.product(args.batch_sizes, args.lrs, args.lambda_clips, args.lambda_pcls, args.temperature_inits, args.affinity_temperatures, args.pcl_temperature_inits):
        tag = f"bs{bs}_lr{lr:g}_lc{lc:g}_lp{lp:g}_temp{temp:g}_at{aff_temp:g}_pt{pcl_temp:g}"
        exps.append({"batch_size": bs, "lr": lr, "lambda_clip": lc, "lambda_pcl": lp, "temperature_init": temp, "affinity_temperature": aff_temp, "pcl_temperature_init": pcl_temp, "tag": tag})
    return exps
...
def make_command(args, exp, output_dir):
    return [sys.executable, "-u", args.train_script,
            ...
            "--affinity_temperature", str(exp["affinity_temperature"]),
...
def write_summary_csv(csv_path, rows):
    fields = ["tag","status","return_code","elapsed_sec","batch_size","lr","lambda_clip","lambda_pcl","temperature_init","affinity_temperature","pcl_temperature_init","best_epoch",
    ...
    with open(combined_log_path, "a", encoding="utf-8") as combined_log:
        ...
        for idx, exp in enumerate(exps, start=1):
            ...
            if args.skip_finished and best_summary_file.exists():
                ...
                row = {"tag": exp["tag"], "status": "skipped_finished", "return_code": 0, "elapsed_sec": 0.0, "batch_size": exp["batch_size"], "lr": exp["lr"], "lambda_clip": exp["lambda_clip"], "lambda_pcl": exp["lambda_pcl"], "temperature_init": exp["temperature_init"], "affinity_temperature": exp["affinity_temperature"], "pcl_temperature_init": exp["pcl_temperature_init"], "output_dir": str(exp_output_dir)}
