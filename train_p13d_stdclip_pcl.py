...
from models.model_p13d_stdclip_pcl import MyModelMDTAP13DSTDCLIPPCL
...
def build_model(args, device):
    return MyModelMDTAP13DSTDCLIPPCL(
...
def train_one_epoch(model, loader, criterion, optimizer, device, args, log_interval=100):
    ...
    for step, batch in enumerate(loader):
        ...
        if (step + 1) % log_interval == 0:
            ...
            print(f"  STEP {step + 1}/{len(loader)} | TOTAL_LOSS={total_loss.item():.6f} | REG_LOSS={reg_loss.item():.6f} | STDCLIP_LOSS={clip_loss.item():.6f} | PCL_LOSS={pcl_loss.item():.6f} | MEAN_GATE={aux['mean_gate'].item():.4f} | BATCH_RMSE={batch_rmse.item():.6f}")
...
def main():
    ...
    parser.add_argument("--output_dir", type=str, default="outputs/davis_p13d_stdclip_pcl")
    ...
    print("MODEL: drug_1d + protein_1d + protein_3d + stdclip(drug-protein) + pcl(protein1d-protein3d)")
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        train_metrics = train_one_epoch(model, train_loader, criterion, optimizer, device, args, log_interval=100)
        val_metrics = evaluate(model, val_loader, criterion, device, args)
        print(f"[EPOCH {epoch:03d}/{args.epochs:03d}] TRAIN: TOTAL={train_metrics['loss']:.6f} | REG={train_metrics['reg_loss']:.6f} | STDCLIP={train_metrics['clip_loss']:.6f} | PCL={train_metrics['pcl_loss']:.6f} | RMSE={train_metrics['rmse']:.6f} | MAE={train_metrics['mae']:.6f} | CI={train_metrics['ci']:.6f} | RM2={train_metrics['rm2']:.6f}")
        print(f"[EPOCH {epoch:03d}/{args.epochs:03d}] VAL  : TOTAL={val_metrics['loss']:.6f} | REG={val_metrics['reg_loss']:.6f} | STDCLIP={val_metrics['clip_loss']:.6f} | PCL={val_metrics['pcl_loss']:.6f} | RMSE={val_metrics['rmse']:.6f} | MAE={val_metrics['mae']:.6f} | CI={val_metrics['ci']:.6f} | RM2={val_metrics['rm2']:.6f}")
