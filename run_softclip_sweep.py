...
def parse_args():
    ...
    parser.add_argument("--train_script", type=str, default="train_p13d_softclip.py")
    ...
    parser.add_argument("--affinity_temperatures", type=float, nargs="+", default=[1.0])

    parser.add_argument("--sweep_name", type=str, default="")
    parser.add_argument("--output_root", type=str, default="outputs/softclip_sweeps")
...
def make_sweep_dir(args):
    if args.sweep_name.strip():
        ...
        sweep_name = "softclip_sweep_" + datetime.now().strftime("%Y%m%d_%H%M%S")
...
def build_experiments(args):
    exps = []
    for bs, lr, lam, temp_init, aff_temp in itertools.product(
        args.batch_sizes, args.lrs, args.lambda_clips, args.temperature_inits, args.affinity_temperatures
    ):
        tag = f"bs{bs}_lr{lr:g}_lc{lam:g}_temp{temp_init:g}_at{aff_temp:g}"
        exps.append({
            "batch_size": bs,
            "lr": lr,
            "lambda_clip": lam,
            "temperature_init": temp_init,
            "affinity_temperature": aff_temp,
            "tag": tag,
        })
    return exps
...
def load_best_metrics(exp_output_dir):
    ...
    return {
        ...
        "val_reg_loss": val.get("reg_loss"),
        "val_clip_loss": val.get("clip_loss"),
        "val_mse": val.get("mse"),
        ...
        "train_reg_loss": train.get("reg_loss"),
        "train_clip_loss": train.get("clip_loss"),
...
def write_summary_csv(csv_path, rows):
    fieldnames = [
        ...
        "batch_size", "lr", "lambda_clip", "temperature_init", "affinity_temperature",
        "best_epoch",
        "val_loss", "val_reg_loss", "val_clip_loss", "val_mse", "val_rmse", "val_mae", "val_ci", "val_rm2",
        "train_loss", "train_reg_loss", "train_clip_loss", "train_mse", "train_rmse", "train_mae", "train_ci", "train_rm2",
        "output_dir",
    ]
        ...
        for idx, exp in enumerate(exps, start=1):
            ...
            if args.skip_finished and best_summary_file.exists():
                ...
                row = {
                    ...
                    "affinity_temperature": exp["affinity_temperature"],
                    "output_dir": str(exp_output_dir),
                }
                if metrics:
                    row.update(metrics)
