...
def parse_args():
    ...
    parser.add_argument("--train_script", type=str, default="train_p13d_pcl.py")
    ...
    parser.add_argument("--lambda_pcls", type=float, nargs="+", default=[0.1])
    parser.add_argument("--temperature_inits", type=float, nargs="+", default=[0.07])

    parser.add_argument("--sweep_name", type=str, default="")
    parser.add_argument("--output_root", type=str, default="outputs/pcl_sweeps")
...
def make_sweep_dir(args):
    if args.sweep_name.strip():
        ...
        sweep_name = "pcl_sweep_" + datetime.now().strftime("%Y%m%d_%H%M%S")
...
def build_experiments(args):
    exps = []
    for bs, lr, lam, temp_init in itertools.product(
        args.batch_sizes, args.lrs, args.lambda_pcls, args.temperature_inits
    ):
        tag = f"bs{bs}_lr{lr:g}_lp{lam:g}_temp{temp_init:g}"
        exps.append({
            "batch_size": bs,
            "lr": lr,
            "lambda_pcl": lam,
...
def load_best_metrics(exp_output_dir):
    ...
    return {
        ...
        "train_pcl_loss": train.get("pcl_loss"),
...
def write_summary_csv(csv_path, rows):
    fieldnames = [
        ...
        "batch_size", "lr", "lambda_pcl", "temperature_init",
        "best_epoch",
        "val_loss", "val_reg_loss", "val_pcl_loss", "val_mse", "val_rmse", "val_mae", "val_ci", "val_rm2",
        "train_loss", "train_reg_loss", "train_pcl_loss", "train_mse", "train_rmse", "train_mae", "train_ci", "train_rm2",
        "output_dir",
    ]
