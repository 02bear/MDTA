...
class MyModelMDTAP13DSOFTCLIPPCL(nn.Module):
    """
    Strategy A: diagonal multiplicative reweight on affinity soft target.
    """
    ...
    def __init__(
        ...
        self.diag_mul_alpha = diag_mul_alpha
    ...
    def compute_softclip_loss(self, drug_feat, protein_feat, affinity_matrix, affinity_mask):
        ...
        target_d2p = self.masked_softmax(affinity_matrix / self.affinity_temperature, affinity_mask, dim=1)
        target_p2d = self.masked_softmax(affinity_matrix.t() / self.affinity_temperature, affinity_mask.t(), dim=1)

        bsz = affinity_matrix.size(0)
        eye = torch.eye(bsz, device=affinity_matrix.device, dtype=affinity_matrix.dtype)
        alpha = max(0.0, float(self.diag_mul_alpha))
        diag_weight = 1.0 + alpha * eye

        target_d2p = target_d2p * diag_weight
        target_d2p = target_d2p * affinity_mask.float()
        target_d2p = target_d2p / target_d2p.sum(dim=1, keepdim=True).clamp_min(1e-12)

        target_p2d = target_p2d * diag_weight.t()
    ...
    def forward(self, batch, affinity_matrix=None, affinity_mask=None, return_aux=True):
        ...
        if return_aux:
            aux = {
                ...
                "diag_mul_alpha": torch.tensor(self.diag_mul_alpha, device=protein_feat.device),
