...
from models.model_p13d_clip import MyModelMDTAP13DCLIP
...
def train_one_epoch(model, loader, criterion, optimizer, device, args, log_interval=50):
    model.train()
    ...
    for step, batch in enumerate(loader):
        ...
        if (step + 1) % log_interval == 0:
            batch_mse = torch.mean((pred - target) ** 2).detach()
            print(
                f"  STEP {step + 1}/{len(loader)} | "
                f"TOTAL_LOSS={total_loss.item():.6f} | "
                f"REG_LOSS={reg_loss.item():.6f} | "
                f"CLIP_LOSS={clip_loss.item():.6f} | "
                f"HARD_NEG_SCORE={aux['mean_valid_neg'].item():.4f} | "
                f"BATCH_MSE={batch_mse.item():.6f}"
            )
...
def main():
    ...
    # train
    parser.add_argument("--output_dir", type=str, default="outputs/davis_p13d_clip_drugclip")
    ...
    print("MODEL: drug_1d + protein_1d + protein_3d + DrugCLIP-style CLIP")

    save_split_indices(train_set, val_set, args.output_dir)

    model = MyModelMDTAP13DCLIP(
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        print(
            ...
            f"CLIP={train_metrics['clip_loss']:.6f} | "
        ...
        print(
            ...
            f"CLIP={val_metrics['clip_loss']:.6f} | "
