...
from models.model_p13d_softclip_pcl import MyModelMDTAP13DSOFTCLIPPCL
...
def train_one_epoch(model, loader, lookup, criterion, optimizer, device, args, log_interval=100):
    ...
    for step, batch in enumerate(loader):
        ...
        if (step + 1) % log_interval == 0:
            ...
            print(f"  STEP {step + 1}/{len(loader)} | TOTAL_LOSS={total_loss.item():.6f} | REG_LOSS={reg_loss.item():.6f} | SOFTCLIP_LOSS={clip_loss.item():.6f} | PCL_LOSS={pcl_loss.item():.6f} | MEAN_GATE={aux['mean_gate'].item():.4f} | MEAN_VALID_PER_ROW={aux['mean_valid_per_row'].item():.2f} | MEAN_VALID_OFFDIAG={aux['mean_valid_offdiag'].item():.2f} | BATCH_RMSE={batch_rmse.item():.6f}")
